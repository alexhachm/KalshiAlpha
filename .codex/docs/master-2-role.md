...
4. Determine validation command and create task, capturing task ID:
   ```bash
   validation_cmd=""
   validation_field=""
...
6. Record request tier/state transition: `./.claude/scripts/codex10 triage <request_id> 2 "Assigned Tier 2 task <task_id>"`.
7. Release claim: `./.claude/scripts/codex10 release-worker "$worker_id"`.
