...
```bash
./.claude/scripts/codex10 queue-research <topic> <question> --mode standard|thinking|deep_research --priority urgent|normal|low --context "..."
```
...
4. **Record Tier-2 triage** (transitions request from pending to decomposed):
   ```bash
   ./.claude/scripts/codex10 triage <request_id> 2 "Assigned Tier 2 task $task_id"
   ```
