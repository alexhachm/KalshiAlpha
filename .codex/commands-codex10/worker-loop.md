...
6. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
7. **Self-verify**: run only explicit build/test/lint commands provided in the task details; if `validation` is shorthand like `tier2`/`tier3`, treat it as metadata (not a shell command) and never assume implicit `npm run build`
