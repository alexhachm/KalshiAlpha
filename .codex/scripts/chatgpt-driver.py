...
Uses a persistent per-project Chrome profile for session reuse.
...
NAMESPACE = (os.getenv("MAC10_NAMESPACE") or "mac10").strip() or "mac10"
PROFILE_HELPER = PROJECT_DIR / "coordinator" / "src" / "research-profile.js"
...
def _slugify_profile_component(value):
    slug = re.sub(r"[^a-z0-9]+", "-", str(value or "project").strip().lower())
    slug = slug.strip("-")[:32]
    return slug or "project"


def _resolve_research_profile_dir():
    override = (os.getenv("MAC10_RESEARCH_PROFILE_DIR") or "").strip()
    if override:
    ...
    project_name = _slugify_profile_component(PROJECT_DIR.name)
    digest = hashlib.sha1(f"{NAMESPACE}:{PROJECT_DIR.resolve()}".encode("utf-8")).hexdigest()[:12]
    return Path.home() / ".chatgpt-codex-profiles" / f"{NAMESPACE}-{project_name}-{digest}"


def _build_profile_process_markers(profile_dir: Path):
    ...
    windows_path = _wsl_to_windows_path(profile_dir)
    if windows_path:
        markers.add(windows_path)
        markers.add(f"--user-data-dir={windows_path}")
    return tuple(marker for marker in markers if marker)

IS_WSL = _is_wsl()
CHROME_BINARY = _find_chrome()
PROFILE_DIR = _resolve_research_profile_dir()
...
def _terminate_profile_chrome_processes(profile_dir: Path):
    ...
    markers = _build_profile_process_markers(profile_dir)
    try:
        ps = subprocess.run(
            ["ps", "-eo", "pid=,args="],
            capture_output=True,
...
def acquire_single_instance_lock():
    """Acquire a process lock so only one research driver runs per project."""
    if fcntl is None:
        return None
