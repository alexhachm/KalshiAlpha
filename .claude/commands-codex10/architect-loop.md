...
You have deep codebase knowledge from the codex10 scan workflow (`.claude/commands-codex10/scan-codebase.md`). Your job is to **triage and act** on requests. You do NOT route Tier 3 tasks to workers — Master-3 handles that.
...
curation_due = false   # Flip true whenever decomposition_count crosses 2.0, 4.0, 6.0...
...
5. Log:
   ...
   ```
   `prev_decomposition_count = decomposition_count`
   `decomposition_count += 0.5`
   `if floor(prev_decomposition_count / 2) < floor(decomposition_count / 2): curation_due = true`
   `last_activity = now()`
...
3. If clarification is needed, use codex10 clarification APIs only:
   ```bash
   ./.claude/scripts/codex10 ask-clarification <request_id> "<question>"
   ```
   Then wait for reply via architect inbox checks (or blocking wait), and resume only after a clarification response is received:
   ```bash
   ./.claude/scripts/codex10 inbox architect
   # or:
   ./.claude/scripts/codex10 inbox architect --block
   ```
4. Create each decomposed task via codex10 (DB-backed queue):
   ```bash
   echo '{"request_id":"[request_id]","subject":"[task title]","description":"REQUEST_ID: [id]\nDOMAIN: [domain]\nFILES: [specific files]\nVALIDATION: tier3\nTIER: 3\n\n[detailed requirements]\n\n[success criteria]","domain":"[domain]","files":["file1.js","file2.js"],"priority":"normal","tier":3,"validation":"tier3","depends_on":[]}' | ./.claude/scripts/codex10 create-task -
   ```
   Repeat once per decomposed task.
5. Verify tasks are allocatable:
   ```bash
   ./.claude/scripts/codex10 ready-tasks
   ```
6. Signal Master-3 wakeup:
   ```bash
...
If `curation_due` (every 2nd decomposition threshold: 2.0, 4.0, 6.0...; Tier 2 contributes 0.5 each):
...
8. Run the codex10 scan workflow by opening and following `.claude/commands-codex10/scan-codebase.md`.
   Do **not** invoke legacy `.claude/commands/scan-codebase.md` in codex10 runtime.
